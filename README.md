...
# 🤖 AI Co-Admin Agent Bot

An advanced Discord server-management bot combining traditional moderation tools with
an AI agent powered by **Google Gemini** (via a free Google AI Studio key). Built 100%
in Python with `discord.py`, source-controlled on **GitHub**, and hosted on a
...
- **Google AI Studio** issues the `GEMINI_API_KEY` that powers every AI command
  (`/ask`, `@mention` chat, `/toxcheck`, `/summarize`, `/moderate`).
...
**AI Agent (Gemini):** `/ask` for direct AI Q&A with rolling memory, `@mention`
...
│   ├── ai_agent.py                 # Gemini-powered AI commands
...
2. Create a free API key → this is your `GEMINI_API_KEY`
3. The free tier has generous but real rate limits — fine for a single-server bot;
   monitor usage in AI Studio if you scale up
...
**Database persistence:** the Space's filesystem resets on every rebuild, so the
SQLite database in `data/` will reset along with it. For persistent warnings/config
across redeploys, either attach a [Hugging Face persistent storage](https://huggingface.co/docs/hub/spaces-storage)
volume to the Space, or migrate `utils/database.py` to an external database
(e.g. Supabase, Postgres, or a hosted SQLite service) later.

**Gemini model alias:** `.env` defaults to `GEMINI_MODEL=gemini-flash-latest`, a
rolling alias Google automatically points at their newest stable Flash model — this
avoids the bot breaking when a specific model version gets deprecated. Pin to an exact
version (e.g. `gemini-2.5-flash`) only if you need fully reproducible output.
