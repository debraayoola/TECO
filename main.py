"""
╔══════════════════════════════════════════════════════════════════╗
║          AI CO-ADMIN AGENT BOT — DISCORD SERVER MANAGEMENT      ║
║          Language: Python | AI: Hugging Face | Deploy: Railway  ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
logger = logging.getLogger('CoAdminBot')

# ─── Bot Intents ──────────────────────────────────────────────────────────────
INTENTS = discord.Intents.all()


class CoAdminBot(commands.Bot):
    def __init__(self):
        super().__init__(
            command_prefix=os.getenv('PREFIX', '!'),
            intents=INTENTS,
            help_command=None,
            description='AI Co-Admin Agent for Discord Server Management'
        )
        self.db = Database()
        self.logger = logger
...
async def help_command(interaction: discord.Interaction):
    embed = discord.Embed(
        title='🤖 AI Co-Admin Bot — Command Reference',
        description='Your intelligent Discord server management assistant, powered by Hugging Face AI.',
    ...
    embed.add_field(
        name='🤖 AI Agent (Hugging Face)',
        value='`/ask` `/analyze` `/toxcheck` `/summarize` `/moderate` `/clearcontext`',
        inline=False
    )
    embed.add_field(
        name='🔧 Auto-Mod',
    ...
    embed.set_footer(text='AI Co-Admin Bot • Powered by Hugging Face | @mention me to chat!')
